...
int main(int argc, char *argv[]) {
	...
	printf("\n\n");printf(" ");
	for(i=0; i<a_length; i++) if(p[i]>=0 && p[i]!=l1[i]-48) printf("%2d", p[i]); else printf("%2c",' ');
	printf("\n  "); 
	for(i=0; i<strlen(l1); i++) printf("%c ",l1[i]);
	printf("\n"); printf("-");  
	for(i=0; i<((a_length-b_length)*2) + 1; i++) printf(" ");
	for(i=0; i<strlen(l2); i++) printf("%c ",l2[i]); printf("\n");	
	for(i = 0; i< a_length*2 + 1; i++) printf("-");printf("\n");
